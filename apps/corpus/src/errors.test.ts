...
import { CORPUS_ERRORS_NOT_FORWARDED, CORPUS_ERROR_MAPPING } from '@holydeck/contracts/corpus';
import { API_ENDPOINTS, SERVER_ERROR_CODES, errorEnvelope, statusForCode } from './errors.js';
...
describe('statusForCode', () => {
  ...
  it('maps a refused internal credential to 401', () => {
    expect(statusForCode('auth_failed')).toBe(401);
  });
...
describe('the boundary the application reads these errors across', () => {
  const decided = new Set([
    ...CORPUS_ERROR_MAPPING.map((entry) => entry.corpus),
    ...CORPUS_ERRORS_NOT_FORWARDED.map((entry) => entry.corpus),
  ]);

  // A code this server can answer with that the boundary has never seen would reach a client as a
  // translation nobody chose, so a new one has to be decided about before it can be answered with.
  it('has a boundary decision for every code it can answer with', () => {
    expect(SERVER_ERROR_CODES.filter((code) => !decided.has(code))).toEqual([]);
  });

  it('has decided about nothing this server cannot answer with, so the boundary stays a description', () => {
    expect([...decided].filter((code) => !SERVER_ERROR_CODES.includes(code as (typeof SERVER_ERROR_CODES)[number])))
      .toEqual([]);
  });
});
