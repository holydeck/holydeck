...
describe('slide-label routes', () => {
  test('creates, validates, and reports duplicate name conflicts by field', async () => { expect((await creating()).statusCode).toBe(201); expect((await creating({ shortcut: '2' })).statusCode).toBe(422); await creating({ name: 'Other', shortcut: '2' }); const duplicate = await creating({ name: 'Praise' }); expect(duplicate.statusCode).toBe(409); expect(duplicate.json().error.code).toBe(ENTITY_CONFLICT); expect(duplicate.json().error.fields).toEqual(expect.arrayContaining([expect.objectContaining({ path: 'name', code: 'field.not_allowed', message: expect.stringContaining('Praise') })])); });
  test('gets, edits, and answers missing or invalid labels', async () => { await creating(); expect((await ask('GET', at(SLIDE_LABEL_ID_PATH, 'label-1'))).statusCode).toBe(200); expect((await ask('GET', at(SLIDE_LABEL_ID_PATH, 'nope'))).statusCode).toBe(404); expect((await ask('PUT', at(SLIDE_LABEL_ID_PATH, 'label-1'), { name: 'Worship', shortcut: '1' })).statusCode).toBe(200); expect((await ask('PUT', at(SLIDE_LABEL_ID_PATH, 'nope'), { name: 'Worship' })).statusCode).toBe(404); expect((await ask('PUT', at(SLIDE_LABEL_ID_PATH, 'label-1'), { shortcut: '1' })).statusCode).toBe(422); });
  test('lists archived entries only on the managed surface and catalogues entries without stamps', async () => { await creating(); await statusing('label-1', true); expect((await ask('GET', SLIDE_LABELS_PATH)).json().data).toHaveLength(1); const editor = await sessions.start(sessionContext('req-editor'), { actor: ACTOR, permissions: [CONTENT_EDIT] }); expect((await ask('GET', SLIDE_LABEL_CATALOGUE_PATH, undefined, editor)).json().data).toEqual([]); await statusing('label-1', false); const entry = (await ask('GET', SLIDE_LABEL_CATALOGUE_PATH, undefined, editor)).json().data[0]; expect(entry).toMatchObject({ id: 'label-1', name: 'Praise' }); expect(entry.stamp).toBeUndefined(); });
  test('archives, unarchives, and refuses an invalid repeated state', async () => { await creating(); expect((await statusing('label-1', true)).json().data.stamp.archivedAt).toBeDefined(); expect((await statusing('label-1', true)).statusCode).toBe(409); expect((await statusing('label-1', false)).json().data.stamp.archivedAt).toBeUndefined(); });
