...
const BINDING = { capabilityId: 'cap-1', kind: 'guest' as const, service: 'service-1', view: 'audience' as const };
// What a socket ticket admits a handshake with: its binding, plus the `LiveGrant` the hub keys a later
// revocation off — the one view it was minted for, watched only, and the capability it came from.
const ADMISSION = { ...BINDING, grant: { ...VIEW_GRANTS.audience, capabilityId: 'cap-1' } };
...
describe('redeeming a socket ticket, which opens one socket and no more (OUT-01)', () => {
  it('opens once, returning the binding it was minted for and the grant that binding opens', () => {
    const { tickets } = harness();
    const minted = tickets.mint({ ...BINDING, capabilityExpiresAt: CAPABILITY_EXPIRES_AT });
    expect(tickets.redeemSocketTicket(minted.socketTicket)).toEqual(ADMISSION);
  });

  it('grants an output window exactly the one view its capability was issued for', () => {
    const { tickets } = harness();
    const binding = { capabilityId: 'cap-3', kind: 'output' as const, service: 'service-1', view: 'stage' as const };
    const minted = tickets.mint({ ...binding, capabilityExpiresAt: CAPABILITY_EXPIRES_AT });
    expect(tickets.redeemSocketTicket(minted.socketTicket)).toEqual({
      ...binding,
      grant: { watch: ['stage'], command: false, capabilityId: 'cap-3' },
    });
  });
...
describe('a revoked capability forgets every ticket it minted (OUT-01)', () => {
  it('invalidates that capability’s own outstanding tickets, and no other capability’s', () => {
    ...
    const otherBinding = { capabilityId: 'cap-2', kind: 'guest' as const, service: 'service-1', view: 'audience' as const };
    ...
    expect(tickets.redeemSocketTicket(other.socketTicket)).toMatchObject(otherBinding);
  });
