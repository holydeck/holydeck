...
// TypeScript file under apps and packages, tests excluded, because a test has to be able to name a
// realistic-looking address to prove the redaction it is testing actually removes one.
...
// RFC1918 only: a public address is not this deployment's own infrastructure to protect, and loopback
// (127.0.0.1, ::1) is the address every stack's own healthcheck names on purpose.
const PRIVATE_IPV4 =
  /\b(?:10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3})\b/gu;
...
export function infraDetailsIn(text) {
  ...
  for (const [at, line] of lines.entries()) {
    for (const pattern of [PRIVATE_IPV4, INTERNAL_HOSTNAME]) {
