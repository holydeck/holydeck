...
test('the repository itself accounts for every permission registration it ships', () => {
  ...
  assert.equal(Object.keys(repo.routeSources).length, 25);
  assert.equal(permissionRoutesIn(repo.routeSources).routes.length, 120);
  assert.deepEqual(verifyRoleCoverage(repo), []);
  assert.equal(verifyRoleCoverage(repo, { tested: {}, gaps: {} }).length, 120);
});
