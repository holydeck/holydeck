...
test.describe('the operator control surface', () => {
  test('takes the kind of input its device has, on the live transport it always renders', async ({ page }, testInfo) => {
    ...
    // The order is empty until a later task wires a real one in, so `control.ts` correctly starts the
    // transport disabled — there is nothing to advance to. `force` bypasses only Playwright's actionability
    // wait for "enabled," not the browser's own native handling of a disabled control, so the input still
    // reaches the surface exactly as a real device would deliver it; the thing graded here is that the
    // surface takes that input at all, on the device's own input kind, without throwing.
